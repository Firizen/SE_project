...
function TeacherDashboard() {
  ...
  const [teacher, setTeacher] = useState({ name: "Unknown", email: "Not Available" });

  useEffect(() => {
    try {
      const storedTeacher = JSON.parse(localStorage.getItem("teacherDetails"));
      ...
      <div className="flex flex-grow">
        ...
        <div className="w-80 bg-gray-800 text-white flex flex-col p-4 space-y-4">
          {[
            ...
            { id: "view", label: "View Active Assignments" },
            { id: "viewPast", label: "View Past Assignments" },
            { id: "viewAppeals", label: "View Appeals" },
          ].map(({ id, label }) => (
            <button
              key={id}
              onClick={() => setActiveSection(activeSection === id ? null : id)}
              className={`py-2 px-4 rounded ${activeSection === id ? "bg-gray-600" : "bg-gray-700"} hover:bg-gray-600 transition`}
            >
              {label}
            </button>
          ))}
