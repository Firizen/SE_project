...
const ViewAppeals = () => {
  ...
  useEffect(() => {
    ...
    const fetchAppeals = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/appeals/teacher`, {
          method: "GET",
          headers: {
  ...
  return (
    <div className="flex space-x-6">
      {!selectedAppeal ? (
              ...
              {appeals.map((appeal) => (
                <li key={appeal._id} className="p-4 border rounded-md shadow-sm bg-gray-50 flex justify-between items-center">
                  <div className="w-8/12">
                    <h3 className="text-lg font-bold">Assignment: {appeal.assignmentTitle || "Untitled"}</h3>
                    <p className="text-gray-600"><strong>Student:</strong> {appeal.studentId?.name || "Unknown"}</p>
                    <p className="text-gray-600"><strong>Status:</strong> {appeal.status || "Pending"}</p>
                  </div>
                  <button
        ...
        <AppealStatus appeal={selectedAppeal} onBack={() => setSelectedAppeal(null)} socket={io("http://localhost:5000")} />
      )}
    </div>
  );
};
