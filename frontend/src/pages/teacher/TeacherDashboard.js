...
function TeacherDashboard() {
  ...
  // State for plagiarism alerts (persisted in localStorage)
  const [plagiarismAlerts, setPlagiarismAlerts] = useState(
    JSON.parse(localStorage.getItem("plagiarismAlerts")) ?? true
  );

  useEffect(() => {
    localStorage.setItem("plagiarismAlerts", JSON.stringify(plagiarismAlerts));
  }, [plagiarismAlerts]);

  // Function to toggle plagiarism alerts
  const togglePlagiarismAlerts = () => {
    setPlagiarismAlerts(!plagiarismAlerts);
  };
    ...
    <div className="min-h-screen flex flex-col bg-gray-100">
      ...
      <div className="flex flex-grow">
        ...
        <div className="w-64 bg-gray-800 text-white flex flex-col p-4 space-y-4">
          ...
          <button 
            onClick={() => setActiveSection(activeSection === "viewAppeals" ? null : "viewAppeals")} 
            className="py-2 px-4 rounded bg-gray-700 hover:bg-gray-600 transition"
          >
            View Appeals
          </button>

          {/* Plagiarism Alert Toggle Button */}
          <button 
            onClick={togglePlagiarismAlerts} 
            className={`py-2 px-4 rounded transition ${plagiarismAlerts ? 'bg-red-600 hover:bg-red-500' : 'bg-green-600 hover:bg-green-500'}`}
          >
            {plagiarismAlerts ? "Disable Plagiarism Alerts" : "Enable Plagiarism Alerts"}
          </button>
        </div>
          ...
          {activeSection === null && (
            <div className="flex content-start">
              <div className="bg-white p-6 rounded-lg shadow-lg w-8/12 mr-20">
                ...
                {/* Show Plagiarism Alert Status */}
                <div className="mt-4 p-4 bg-gray-200 rounded-md">
                  <p className="text-lg font-semibold">
                    Plagiarism Alerts: 
                    <span className={plagiarismAlerts ? "text-green-600" : "text-red-600"}>
                      {plagiarismAlerts ? " Enabled ✅" : " Disabled ❌"}
                    </span>
                  </p>
                </div>
              </div>
