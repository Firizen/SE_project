...
function StudentDashboard() {
  ...
  const [showNotifications, setShowNotifications] = useState(true); // New state to control notifications visibility
  ...
  const handleLogout = () => {
    ...
    window.location.href = "/student/login"; // Change this route based on your login page
  };
                ...
                {showNotifications && notifications.length > 0 ? (
                  <ul className="list-disc pl-5 mt-4">
                    {notifications.map((notif) => (
                      <li key={notif._id} className="text-gray-800">
                        {notif.message}{" "}
